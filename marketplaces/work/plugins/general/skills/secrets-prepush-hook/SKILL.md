...
description: >-
  ...
  (the gitleaks successor) using its expressions-based config before they leave the machine. Use when
  asked to set up a secret-scanning pre-push hook, prevent committing/pushing secrets, or harden a repo's
  git workflow. Local fast-feedback layer; the hard CI gate stays kingfisher (pipeline-conventions).
---
...
(kingfisher, see `pipeline-conventions`). The hook ships with this plugin at `git-hooks/pre-push` and
auto-uses the repo's `.betterleaks.toml` expressions config (template at `templates/betterleaks.toml`).
...
### Step 2 — Install the hook + config (team-shareable, [CONFIRM])
...
cp <plugin>/templates/betterleaks.toml .betterleaks.toml # expressions config (Expr filter/validate)
git config core.hooksPath .githooks                      # repo-local; commit .githooks/ + .betterleaks.toml to share
```
...
- Runs `betterleaks git . --config .betterleaks.toml --redact` → uses the Expr filter/validate config; secrets are never printed.
- Secret found → push **blocked** (exit 1); refine the `filter` expression in `.betterleaks.toml`, or bypass once with `git push --no-verify`.
