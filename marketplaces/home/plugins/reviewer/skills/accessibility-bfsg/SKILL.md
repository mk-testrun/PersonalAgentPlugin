...
description: Nutze um WCAG-Befunde auf BFSG/BITV 2.0 zu mappen und eine publizierfähige „Erklärung zur Barrierefreiheit" (§12 BFSG) zu erzeugen — für .de-Domains besonders relevant.
---

## Scope

Rechtliches Mapping deutscher Barrierefreiheit. Voraussetzung: WCAG-Befunde aus
`accessibility-wcag`. Erzeugt zusätzlich die gesetzlich geforderte Erklärung (ersetzt den früheren accessibility-report).
...
3. **BFSG-CONFORM** — Konformitätsstatus „vollständig | teilweise | nicht" begründet. *(high bei „nicht konform")*
4. **BFSG-FEEDBACK** — Feedback-Mechanismus + Kontakt für Barriere-Meldungen. *(medium)*
5. **BFSG-ENFORCE** — Hinweis auf Durchsetzungsverfahren/Schlichtungsstelle. *(low)*
...
**Stand:** <YYYY-MM-DD>  ·  **Basis:** Selbstbewertung / WCAG-2.2-Prüfung

## Konformitätsstatus
<vollständig | teilweise | nicht> konform mit BITV 2.0 / EN 301 549.

## Nicht barrierefreie Inhalte
- <Befund + WCAG-Kriterium + geplante Behebung/Datum>
...
Ausgabe als Markdown-Artefakt nach `state/artifacts/`.

## Output

findings[] nach `docs/findings-schema.md`, `area: accessibility`, ruleId aus `BFSG-*`,
**plus** die fertige Erklärung als Markdown. Bei `critical`/`high`: **[GATE]**.
